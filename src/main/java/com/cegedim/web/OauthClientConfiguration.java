...
public class OauthClientConfiguration {
	...
	/*
	 * ClientHttpRequestFactory is autowired and checked in case somewhere in your
	 * configuration you provided {@link ClientHttpRequestFactory} implementation
	 * Bean where you defined specifics of your connection, if not it is
	 * instantiated here with {@link SimpleClientHttpRequestFactory}
	 */
	...
	public OAuth2RestOperations restClientOnlyTemplate(
		...
		OAuth2RestTemplate template = new OAuth2RestTemplate(fullAccessresourceDetailsClientOnly(tokenUrl),
				new DefaultOAuth2ClientContext(new DefaultAccessTokenRequest()));
	...
	/**
	 * {@link AccessTokenProviderChain} throws InsufficientAuthenticationException
	 * in obtainAccessToken(OAuth2ProtectedResourceDetails resource,
	 * AccessTokenRequest request) if user is not authorized, but since we are
	 * setting our own accessTokenProvider() on OAuth2RestTemplate this condition is
	 * not being checked, thus exception is not being thrown and requirement for
	 * user to be logged in is skipped
	 */
	@Bean
	...
	private OAuth2ProtectedResourceDetails fullAccessresourceDetailsClientOnly(String tokenUrl) {
